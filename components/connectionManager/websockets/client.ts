...
/**
 * Unified WebSocket Client for Entity Manager v2
 *
 * Combines robust connection management with auto-reconnect,
 * heartbeat monitoring, message queuing, and real-time updates.
 */
 ...
 */
export function createWebSocketClient(config: ConnectionConfig, callbacks: ConnectionCallbacks = {}): WebSocketClient {
  return new WebSocketClient(config, callbacks);
}

/**
 * Create an entity-specific WebSocket client
 */
export function createEntityWebSocketClient(
  entityType: string,
  ...
  return createWebSocketClient({ url }, callbacks);
}
