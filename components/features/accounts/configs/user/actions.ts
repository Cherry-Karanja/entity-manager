  ...
  item: [
    {
      ...
      href(item) {
        ...
        return `/dashboard/accounts/users/${user.id}`
      },
    ...
    },
    {
      ...
      actionType: 'modal',
      modal: {
        title: (item: unknown) => `Change Role for ${(item as User).full_name || (item as User).email}`,
        content: ChangeRoleModal,
        width: 500,
      },
      onExecute: async (item: unknown) => {
        const user = item as User
        console.log('Opening role change modal for user:', user.id)
        // The modal will handle the role change
      }
    },
    {
      id: 'toggle_status',
    ...
    },
    {
      ...
      href(item) {
        const user = item as User
        return `/accounts/user-sessions?user_id=${user.id}`
      },
      onExecute: async (item: unknown) => {
        const user = item as User
    ...
    },
    {
      ...
      href(item) {
        const user = item as User
        return `/accounts/login-attempts?user_id=${user.id}`
      },
      onExecute: async (item: unknown) => {
        const user = item as User
