...
export const usePermissions = () => {
  ...
  const userPermissions = useMemo(() => {
    if (!user?.user_type || !isAuthenticated) return []

    return USER_TYPE_PERMISSIONS[user.user_type as keyof typeof USER_TYPE_PERMISSIONS] || []
  }, [user?.user_type, isAuthenticated])
