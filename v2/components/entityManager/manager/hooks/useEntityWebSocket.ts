...
import { useNotificationWebSocket, useEntityWebSocket } from '@/components/connectionManager/websockets';
...
export const useEntityNotificationWebSocket = () => {
  ...
  useEffect(() => {
    if (AuthAPI.isAuthenticated() && !ws.isConnected) {
      ws.connect();
    } else if (!AuthAPI.isAuthenticated() && ws.isConnected) {
      ws.disconnect();
    }
  }, [AuthAPI.isAuthenticated(), ws.isConnected, ws.connect, ws.disconnect]);
